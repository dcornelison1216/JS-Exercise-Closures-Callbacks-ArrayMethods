...
function processSum(numberList, callback) {
  const reduceNumberList = numberList.reduce((total, num) => {
    return total += num;
  }, 0);
  return callback(reduceNumberList);
}
...
function processContains(item, list, callback) {
  const filterList = list.filter(check => check == item);
...
function getRunnersByTShirtSize(runners, tShirtSize) {
  const filterRunners = runners.filter((item, index) => {
    return runners[index].shirt_size === tShirtSize
  });
...
function tallyUpDonations(runners) {
  const donationArray = [];
  runners.forEach((object, index, arr) => {
    donationArray.push(object.donation)
  });
  const reducer = (accumulator, currentValue) => accumulator + currentValue;
  const sum = donationArray.reduce(reducer, 0);
  return sum;
}
